...
model_path = os.path.join(base_dir, 'linear_svm_tuned_model.pkl')
...
def clean_and_extract_review_body(chunk_text):
    """Advanced Regex Metadata Stripping"""
...
def analyze():
    ...
    scraped_data = []
    results = []      # 🌟 Perbaikan: Inisialisasi list untuk menyimpan hasil prediksi
    bot_count = 0     # 🌟 Perbaikan: Inisialisasi counter untuk bot
    
    # --- LOGIKA SELEKSI INPUT METODE (Sekarang masuk ke dalam indentasi fungsi) ---
    if input_type == 'link':
        if not SCRAPER_AVAILABLE:
            return render_template(
                ...
                results=[
                    {
                        "text": "URL scraping is unavailable in this deployment. Please use Manual Input.",
                        "label": "INFO",
                        "indications": "-"
                    }
                ]
    ...
    # --- PROSES PREDIKSI UNTUK KEDUA METODE INPUT ---
    if isinstance(scraped_data, list) and len(scraped_data) > 0:
        ...
        # Fallback jika user memasukkan pola teks yang merusak struktur segmentasi / scraper kosong
        percentage = 0
        human_percentage = 100
        results.append({
            'text': "Gagal memproses data atau ulasan kosong. Pastikan input Anda valid.",
            'label': "ERROR",
            'indications': "-"
        })
